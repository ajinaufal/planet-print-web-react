...
export function SidebarLayout() {
    return (
        <aside className="flex flex-col gap-3 bg-target-primary-hover h-screen w-fit min-w-52 pt-8">
            <img src={ImageConstants.beautyProfile} className="w-36 px-4 mx-auto mb-4" />
            {(SidebarMenus || []).map((menu, index) => (
                <Link
                    id={index}
                    className="text-white hover:bg-background-secondary hover:text-black rounded-l-lg px-4 py-2"
                    // style={{
                    //   WebkitMaskImage:
